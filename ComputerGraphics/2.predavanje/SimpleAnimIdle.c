...
void myKeyboardFunc( unsigned char key, int x, int y )
{
	switch ( key ) {
	case 'r':
		RunMode = 1-RunMode;		// Toggle to opposite 
//value
		if ( RunMode==1 ) {
			 glutIdleFunc(drawIdle);
		}
		else {
			 glutIdleFunc(NULL);			//NULL salje nulu
		}
		break;
	case 's':
		drawIdle();
...
void drawIdle(void)										//ovo je razlika od simpleAnim sto ovo ima idle funkciju
{
